...
首批内置课程使用同一套 Course Pack、学习引擎和客户端交互承载不同语言。版本 0.4.0 将每门课程扩展为 12 课 A1 核心路径。课程覆盖 A1 的主要生活能力域，但尚不宣称通过外部考试认证或穷尽 A1 的全部词汇与任务。
...
每个语种包含 12 个 Can-do 目标和 36 个独立练习。讲解、翻译、提示与选项同时提供中文和英文；目标语言文本不随应用语言切换。基础课只做文字、标音识别、排序和输入，不包含录音、语音识别或发音评分。

## 学习顺序

课节按数组顺序逐课解锁，前四课构成文字／标音基础模块，中间五课构成 A1 高频能力模块，后三课构成服务场景迁移模块。前四课提供通用基础诊断：通过后沿短分支完成，未通过则进入正常教学；协议与播放器都不判断具体语种。
...
自动测试检查课程引用和分支流程有效性、学习路径可达性、诊断目标合法性、每课练习独立性、四种练习形态覆盖、读音存在，以及所有教学字段的中英文完整性。详细能力域和日语考试差距见 [A1 核心课程地图](A1_CURRICULUM.md)。
