...
现有学习、创作、语言扩展、离线、备份和跨客户端架构已经形成可用闭环。R33 负责补齐四个核心缺口：用户控制的数据删除与学习重置、课程级结业总结、从既有证据派生的薄弱项练习，以及本地数据故障恢复。统一设置入口和完整回归测试随 R33 一起完成。
...
- 无账户也可完整学习；课程、进度和草稿可以备份与迁移，R33 将补齐完整删除控制。
