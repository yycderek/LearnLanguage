...
import type { CourseLearningRecord } from "./learning-record.js";
...
export function courseAdaptiveAgenda(
  ...
  return buildAdaptiveLearningAgenda(
    plan,
    {
      ...
      dueReviewCount: record ? record.reviews.filter((item) => Date.parse(item.dueAt) <= Date.parse(occurredAt)).length : 0,
