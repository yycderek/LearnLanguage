...
test("Studio tools and language filters work by keyboard without losing the draft", async ({ page }, testInfo) => {
  await page.goto(origin + "/studio");
  await dismissFirstUseGuide(page);
  const search = page.getByRole("searchbox", { name: "筛选目标语言", exact: true });
  await search.fill("Español");
  await expect(page.locator(".studio-ready-languages aside button")).toHaveCount(1);
  await search.fill("no-such-language");
  await expect(page.getByText("没有匹配语言，请更换关键词。")).toBeVisible();
  await page.getByRole("button", { name: "清除筛选", exact: true }).click();
  await search.fill("英语");
  await page.locator(".studio-ready-languages aside button").click();
  const tools = page.locator(".studio-tools summary");
  await tools.focus();
  await page.keyboard.press("Enter");
  await page.getByRole("button", { name: "AI 设置", exact: false }).focus();
  await page.keyboard.press("Tab");
  await expect(page.getByRole("button", { name: "素材生成课程", exact: true })).toBeFocused();
  ...
  await tools.focus();
  await page.keyboard.press("Enter");
  await expect(page.getByRole("button", { name: "AI 设置", exact: false })).toBeVisible();
  await page.keyboard.press("Escape");
  await expect(tools).toBeFocused();
  await expect(page.getByRole("button", { name: "使用帮助", exact: true })).toBeHidden();
  await tools.press("Enter");
  await page.getByRole("button", { name: "使用帮助", exact: true }).click();
  await page.getByRole("button", { name: "关闭使用帮助", exact: true }).click();
  await expect(tools).toBeFocused();
  await tools.press("Enter");
  await page.getByRole("combobox", { name: "界面与讲解语言", exact: true }).selectOption("en");
  await page.keyboard.press("Escape");
  await expect(page.getByRole("button", { name: "Save draft", exact: true })).toBeVisible();
  await tools.press("Enter");
  await page.getByRole("combobox", { name: "Interface and instruction language", exact: true }).selectOption("zh-CN");
  await page.keyboard.press("Escape");
  await expectResponsiveDocument(page);
  await page.locator(".topbar").screenshot({ path: testInfo.outputPath("studio-tools.png") });
  await page.locator("button:visible").filter({ hasText: /语言包管理|^语言包$/ }).first().click();
  await page.getByRole("searchbox", { name: "筛选目标语言", exact: true }).fill("ja");
  await expect(page.locator(".language-pack-card")).toHaveCount(1);
...
test("draft actions protect unsaved edits and return focus on cancel", async ({ page }, testInfo) => {
  await page.goto(origin + "/studio");
  await dismissFirstUseGuide(page);
  await page.getByRole("searchbox", { name: "筛选目标语言" }).fill("English");
  await page.locator(".studio-ready-languages aside button").click();
  const title = page.getByLabel("课程名称（中文）", { exact: true });
  await title.fill("要保留的课程");
  await page.getByRole("button", { name: "保存草稿", exact: true }).click();
  await expect(page.getByText("已保存到当前设备 · 修订 1", { exact: true })).toBeVisible();
  await title.fill("尚未保存的编辑");
  await page.locator("button:visible").filter({ hasText: /^本地草稿/ }).first().click();
  const restore = page.getByRole("button", { name: "继续编辑", exact: true });
  await restore.click();
  const dialog = page.getByRole("dialog");
  await expect(dialog).toHaveAccessibleName("替换未保存的编辑内容？");
  await expect(dialog).toBeFocused();
  await page.keyboard.press("Tab");
  await expect(dialog.getByRole("button", { name: "取消", exact: true })).toBeFocused();
  await page.keyboard.press("Shift+Tab");
  await expect(dialog.getByRole("button", { name: "替换并恢复" })).toBeFocused();
  await expectResponsiveDocument(page);
  await dialog.screenshot({ path: testInfo.outputPath("restore-confirm.png") });
  await page.keyboard.press("Escape");
  await expect(restore).toBeFocused();
  await page.getByRole("button", { name: "返回课程编辑器" }).click();
  await expect(title).toHaveValue("尚未保存的编辑");
  await page.locator("button:visible").filter({ hasText: /^本地草稿/ }).first().click();
  await restore.click();
  await dialog.getByRole("button", { name: "替换并恢复" }).click();
  await expect(title).toHaveValue("要保留的课程");
  await page.locator("button:visible").filter({ hasText: /^本地草稿/ }).first().click();
  const remove = page.getByRole("button", { name: "删除要保留的课程", exact: true });
  await remove.click();
  await expect(dialog).toHaveAccessibleName("删除这份草稿？");
  await expect(dialog).toContainText("全部 1 个本地修订");
  await expectResponsiveDocument(page);
  ...
  await expect(page.locator(".draft-card")).toHaveCount(1);
  await remove.click();
  await dialog.getByRole("button", { name: "删除全部修订" }).click();
  await expect(dialog).toBeHidden();
  await expect(page.getByRole("heading", { name: "管理课程草稿" })).toBeFocused();
  await expect(page.getByRole("heading", { name: "还没有本地草稿" })).toBeVisible();
  await page.getByRole("button", { name: "返回课程编辑器" }).click();
  await expect(title).toHaveValue("要保留的课程");
});

test("draft import protects unsaved content and reports invalid files", async ({ page }, testInfo) => {
  await page.goto(origin + "/studio");
  await dismissFirstUseGuide(page);
  await page.getByRole("searchbox", { name: "筛选目标语言" }).fill("English");
  await page.locator(".studio-ready-languages aside button").click();
  const title = page.getByLabel("课程名称（中文）", { exact: true });
  await title.fill("未保存的导入前内容");
  await page.locator("button:visible").filter({ hasText: /^本地草稿/ }).first().click();
  const fileInput = page.getByLabel("选择草稿文件", { exact: true });
  const imported = sampleCourse("en", "English");
  imported.manifest.title = { "zh-CN": "导入后的课程", en: "Imported course" };
  const file = { name: "travel.draft.json", mimeType: "application/json", buffer: Buffer.from(JSON.stringify(imported)) };
  await fileInput.setInputFiles(file);
  const dialog = page.getByRole("dialog");
  await expect(dialog).toContainText("travel.draft.json");
  await expect(dialog).toContainText("替换当前未保存的编辑内容");
  await expectResponsiveDocument(page);
  await dialog.screenshot({ path: testInfo.outputPath("import-confirm.png") });
  await page.keyboard.press("Escape");
  await expect(page.getByRole("button", { name: "导入草稿文件", exact: true })).toBeFocused();
  await expect(page.locator(".draft-card")).toHaveCount(0);
  await page.getByRole("button", { name: "返回课程编辑器" }).click();
  await expect(title).toHaveValue("未保存的导入前内容");
  await page.locator("button:visible").filter({ hasText: /^本地草稿/ }).first().click();
  await fileInput.setInputFiles({ name: "invalid.json", mimeType: "application/json", buffer: Buffer.from("invalid") });
  await dialog.getByRole("button", { name: "导入并替换" }).click();
  await expect(dialog.getByRole("alert")).toContainText("草稿文件格式无效");
  await dialog.getByRole("button", { name: "取消", exact: true }).click();
  await page.getByRole("button", { name: "返回课程编辑器" }).click();
  await expect(title).toHaveValue("未保存的导入前内容");
  await page.locator("button:visible").filter({ hasText: /^本地草稿/ }).first().click();
  await fileInput.setInputFiles(file);
  await dialog.getByRole("button", { name: "导入并替换" }).click();
  await expect(title).toHaveValue("导入后的课程");
  await page.locator("button:visible").filter({ hasText: /^本地草稿/ }).first().click();
  await expect(page.locator(".draft-card")).toHaveCount(1);
  await fileInput.setInputFiles({ name: "invalid.json", mimeType: "application/json", buffer: Buffer.from("invalid") });
  await expect(dialog).toBeHidden();
