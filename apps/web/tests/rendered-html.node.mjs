...
test("build contains the visual course studio and language pack workflow", async () => {
  const [page, learnPage, studioPage, readme, userGuide, studio, flowEditor, contentEditor, studioStart, materialImportDialog, productGuide, dialogFocus, learnRoute, draftManager, languagePackManager, player, renderer, dashboard, courseLibraryPage, reviewPlayer, css, languagePack, languagePackFile, starterLibrary, courseLibrary, courseFile, draftLibrary, learnerBackup, deviceBackup, learning, courseAuthoring, courseTemplates, publishReadiness, deviceSync, deviceRepository, applicationWorkspace, applicationAuthoring, languageRuntime, ai, aiRoute, worker, nextConfig] = await Promise.all([
    ...
    readFile(new URL("../../packages/content/src/language-pack.ts", root), "utf8"),
    readFile(new URL("lib/language-pack-file.ts", root), "utf8"),
    readFile(new URL("../../packages/content/src/starter-course-library.ts", root), "utf8"),
    ...
    readFile(new URL("../../packages/application/src/learning-record.ts", root), "utf8"),
  ...
  assert.match(flowEditor, /添加课节/);
  assert.match(studio, /selectedStudioLessonId/);
  assert.match(flowEditor, /课程课节顺序/);
  assert.match(flowEditor, /moveLessonStep/);
  assert.match(flowEditor, /课程单元与课节/);
  assert.match(flowEditor, /ReferencePicker/);
  ...
  assert.match(studio, /learningView === "settings"/);
  assert.match(studio, /resetCurrentCourseLearning/);
  assert.match(studio, /deleteAllLocalData/);
  assert.match(studio, /exportStorageDiagnostics/);
  assert.match(studio, /rebuildLocalStorage/);
  assert.match(dashboard, /summarizeCourseCompletion/);
  assert.match(dashboard, /course-completion-card/);
  assert.match(dashboard, /weak-knowledge-panel/);
  assert.match(courseLibraryPage, /settingsMode/);
  assert.match(courseLibraryPage, /settings-center-overview/);
  assert.match(deviceRepository, /resetCourseDeviceData/);
  assert.match(deviceRepository, /clearAllDeviceData/);
  assert.match(deviceRepository, /diagnoseDeviceStorage/);
  assert.match(deviceRepository, /rebuildDeviceDatabase/);
  assert.match(css, /settings-center-overview/);
  assert.match(css, /course-completion-card/);
  ...
  assert.match(flowEditor, /duplicateLesson/);
