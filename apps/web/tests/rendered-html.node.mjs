...
test("build contains the visual course studio and language pack workflow", async () => {
  ...
  assert.match(studio, /UI_LOCALE_PREFERENCE_KEY/);
  assert.match(studio, /Course content editor/);
  assert.match(studio, /Interface/);
  assert.match(studio, /Teaching/);
  ...
  assert.match(dashboard, /onUiLocaleChange/);
  assert.match(dashboard, /uiText\(uiLocale/);
  ...
  assert.match(reviewPlayer, /uiText\(uiLocale/);
