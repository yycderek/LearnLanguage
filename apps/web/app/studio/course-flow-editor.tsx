...
<section className="unit-manager" aria-label={t("课程单元", "Course units")}>
                        {(course.units ?? []).map((unit, index) => (
                          <article className={unit.id === selectedUnitId ? "unit-card active" : "unit-card"} key={unit.id}>
                            ...
                            <label><small>{t("单元名称", "Unit title")}</small><input readOnly={readOnly} value={unit.title[teachingLocale] ?? ""} onFocus={() => setSelectedUnitId(unit.id)} onChange={(event) => editCourse((next) => { renameUnit(next, unit.id, teachingLocale, event.target.value); })} /></label>
                            <em>{t(unit.lessonRefs.length + " 课", unit.lessonRefs.length + " lessons")}</em>
                            <div className="unit-actions"><button type="button" onClick={(event) => { event.stopPropagation(); moveCourseUnit(unit.id, -1); }} disabled={readOnly || index === 0} aria-label={t("单元前移", "Move unit earlier")}><ArrowUp size={14} /></button><button type="button" onClick={(event) => { event.stopPropagation(); moveCourseUnit(unit.id, 1); }} disabled={readOnly || index === (course.units?.length ?? 0) - 1} aria-label={t("单元后移", "Move unit later")}><ArrowDown size={14} /></button><button type="button" className="danger" onClick={(event) => { event.stopPropagation(); deleteCourseUnit(unit.id); }} disabled={readOnly || (course.units?.length ?? 0) <= 1} aria-label={t("删除单元", "Delete unit")}><Trash2 size={14} /></button></div>
                          </article>
                        ))}
                      </section>
...
<div className="lesson-sequence" role="group" aria-label={t("课程课节顺序", "Course lesson order")}>
                      ...
                      {selectedStudioLesson && <div className="selected-lesson-panel">
                        <div className="selected-lesson-heading"><div><span>{t((readOnly ? "正在查看第 " : "正在编辑第 ") + (selectedStudioLessonIndex + 1) + " 课", (readOnly ? "Viewing lesson " : "Editing lesson ") + (selectedStudioLessonIndex + 1))}</span><strong>{displayText(selectedStudioLesson.title, teachingLocale)}</strong></div><div><button type="button" disabled={readOnly} onClick={duplicateCourseLesson} aria-label={t("复制当前课节", "Duplicate current lesson")}><Copy size={15} /></button><button type="button" onClick={() => moveCourseLesson(-1)} disabled={readOnly || selectedStudioLessonIndex === 0} aria-label={t("课节前移", "Move lesson earlier")}><ArrowUp size={15} /></button><button type="button" onClick={() => moveCourseLesson(1)} disabled={readOnly || selectedStudioLessonIndex === course.lessons.length - 1} aria-label={t("课节后移", "Move lesson later")}><ArrowDown size={15} /></button><button type="button" className="danger" onClick={deleteCourseLesson} disabled={readOnly || course.lessons.length <= 1} aria-label={t("删除当前课节", "Delete current lesson")}><Trash2 size={15} /></button></div></div>
                        <div className="form-grid two-column lesson-fields">
                          <label><span>{t("课节名称", "Lesson title")} ({teachingLocale === "en" ? "English" : "中文"})</span><input readOnly={readOnly} value={selectedStudioLesson.title[teachingLocale] ?? ""} onChange={(event) => editCourse((next) => { const lesson = selectedDraftLesson(next); if (lesson) lesson.title[teachingLocale] = event.target.value; })} /></label>
                          <label><span>{t("所属单元", "Unit")}</span><select disabled={readOnly} value={(course.units ?? []).find((unit) => unit.lessonRefs.includes(selectedStudioLesson.id))?.id ?? ""} onChange={(event) => setLessonUnit(selectedStudioLesson.id, event.target.value)}>{(course.units ?? []).map((unit) => <option key={unit.id} value={unit.id}>{displayText(unit.title, teachingLocale)}</option>)}</select></label>
                          <ReferencePicker disabled={readOnly} locale={locale} label={t("本课学习目标", "Learning goals for this lesson")} options={goalOptions} selected={selectedStudioLesson.canDoGoalRefs} emptyLabel={t("暂无能力目标，可在 JSON 进阶模式补充", "No can-do goals yet; add them in advanced JSON mode")} onChange={(ids) => editCourse((next) => { const lesson = selectedDraftLesson(next); if (lesson) lesson.canDoGoalRefs = ids; })} />
                        </div>
                      </div>}
                      <div className={styles.stepsHeading}><h4 className={styles.sectionTitle}>{t("学习步骤", "Learning steps")} · {flow.length}</h4><button type="button" className="outline-button" onClick={addStep} disabled={readOnly || !selectedStudioLesson}><Plus size={15} />{t("添加步骤", "Add step")}</button></div>
...
<div className={styles.stepBody}>
                            <div className="form-grid two-column">
                              <label><span>{t("阶段", "Phase")}</span><select disabled={readOnly} value={step.phase} onChange={(event) => editCourse((next) => { const lesson = selectedDraftLesson(next); if (lesson) lesson.steps[index].phase = event.target.value as LessonPhase; })}><option value="diagnostic">{t("诊断", "Diagnostic")}</option><option value="preteach">{t("预教", "Pre-teaching")}</option><option value="supported-input">{t("支持性输入", "Supported input")}</option><option value="comprehension">{t("独立理解", "Comprehension")}</option><option value="guided-output">{t("引导输出", "Guided output")}</option><option value="independent-task">{t("独立任务", "Independent task")}</option><option value="feedback-retry">{t("反馈重试", "Feedback retry")}</option><option value="delayed-transfer">{t("延迟迁移", "Delayed transfer")}</option></select></label>
                              <label><span>{t("显示标题", "Display title")} ({teachingLocale === "en" ? "English" : "中文"})</span><input readOnly={readOnly} ref={(element) => { const key = selectedStudioLesson.id + ":" + step.id; if (element && pendingFocus.current === key) { element.focus(); pendingFocus.current = undefined; } }} value={step.title[teachingLocale] ?? ""} onChange={(event) => editCourse((next) => { const lesson = selectedDraftLesson(next); if (lesson) lesson.steps[index].title[teachingLocale] = event.target.value; })} /></label>
                              <ReferencePicker disabled={readOnly} locale={locale} label={t("本步骤知识点", "Knowledge in this step")} options={knowledgeOptions} selected={step.knowledgeRefs} emptyLabel={t("暂无知识点", "No knowledge yet")} onChange={(ids) => editCourse((next) => { const lesson = selectedDraftLesson(next); if (lesson) lesson.steps[index].knowledgeRefs = ids; })} />
                              <ReferencePicker disabled={readOnly} locale={locale} label={t("本步骤例句", "Utterances in this step")} options={utteranceOptions} selected={step.utteranceRefs} emptyLabel={t("暂无例句", "No utterances yet")} onChange={(ids) => editCourse((next) => { const lesson = selectedDraftLesson(next); if (lesson) lesson.steps[index].utteranceRefs = ids; })} />
                              <ReferencePicker disabled={readOnly} locale={locale} label={t("本步骤练习", "Exercises in this step")} options={exerciseOptions} selected={step.exerciseRefs} emptyLabel={t("暂无练习", "No exercises yet")} onChange={(ids) => editCourse((next) => { const lesson = selectedDraftLesson(next); if (lesson) lesson.steps[index].exerciseRefs = ids; })} />
                            </div>
                            <div className="step-actions"><button type="button" onClick={() => moveStep(index, -1)} disabled={readOnly || index === 0} aria-label={t("上移步骤 " + (index + 1), "Move step " + (index + 1) + " up")}><ArrowUp size={14} /></button><button type="button" onClick={() => moveStep(index, 1)} disabled={readOnly || index === flow.length - 1} aria-label={t("下移步骤 " + (index + 1), "Move step " + (index + 1) + " down")}><ArrowDown size={14} /></button><button type="button" className="danger" onClick={() => removeStep(index)} disabled={readOnly || flow.length <= 1} aria-label={t("删除步骤 " + (index + 1), "Delete step " + (index + 1))}><Trash2 size={14} /></button></div>
                          </div></details>
                        ))}
                      </div>
                    </div>
