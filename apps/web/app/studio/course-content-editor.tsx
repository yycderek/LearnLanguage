      ...
      <div className="form-section">
        ...
        <div className="item-stack">
          {course.utterances.map((item, index) => (
            <article className="edit-card" key={item.id}>
              <div className="edit-card-heading"><button type="button" className={styles.toggle} aria-expanded={!collapsed[item.id]} onClick={() => setCollapsed((previous) => ({ ...previous, [item.id]: !previous[item.id] }))}><ChevronDown size={16} /><span>{t(`例句 ${index + 1}`, `Utterance ${index + 1}`)}</span><span>{item.text || t("未命名", "Untitled")}</span></button><button onClick={() => removeUtterance(index)} aria-label={t(`删除例句 ${index + 1}`, `Delete utterance ${index + 1}`)}><Trash2 size={15} /></button></div>
              <div className="form-grid two-column" hidden={collapsed[item.id]}>
                <ReferencePicker locale={locale} label={t("关联知识点", "Related knowledge")} options={knowledgeOptions} selected={item.knowledgeRefs} emptyLabel={t("请先添加知识点", "Add knowledge first")} onChange={(ids) => editCourse((next) => { next.utterances[index].knowledgeRefs = ids; })} />
          ...
          {course.exercises.map((item, index) => ({ item, index })).filter(({ item }) => matches(item)).map(({ item, index }) => (
            <article className="edit-card" key={item.id}>
              ...
              <div className="form-grid two-column" hidden={collapsed[item.id]}>
                ...
                <ReferencePicker locale={locale} label={t("练习涉及的知识点", "Knowledge used by this exercise")} options={knowledgeOptions} selected={item.knowledgeRefs} emptyLabel={t("暂无知识点，可稍后添加", "No knowledge yet; you can add it later")} onChange={(ids) => editCourse((next) => { next.exercises[index].knowledgeRefs = ids; })} />
                <ReferencePicker locale={locale} label={t("练习使用的例句", "Utterances used by this exercise")} options={utteranceOptions} selected={item.utteranceRefs} emptyLabel={t("请先添加例句", "Add utterances first")} onChange={(ids) => editCourse((next) => { next.exercises[index].utteranceRefs = ids; })} />
