...
import { readLearnRoute, writeLearnRoute } from "@/lib/learn-route";
import {
  clearAllDeviceData,
...
import { createCourseDraftFromMaterials, MAX_MATERIAL_CHARACTERS } from "@/lib/material-course";
import { extractMaterialFile, fetchMaterialUrl, MAX_MATERIALS, type CourseMaterial } from "@/lib/material-import";
...
import type { MaterialImportForm } from "@/app/material-import-dialog";

const MaterialImportDialog = dynamic(
  () => import("@/app/material-import-dialog").then((module) => module.MaterialImportDialog),
  { ssr: false },
);
...
export function CourseStudio({ space = "studio" }: { space?: "learn" | "studio" }) {
  ...
  useEffect(() => {
    ...
    async function hydrate() {
      ...
      if (space === "learn" && storedInstalledCourses[0]) {
        const requestedRoute = readLearnRoute(window.location.search);
        const firstCourse = storedInstalledCourses.find((item) => item.manifest.id === requestedRoute.courseId) ?? storedInstalledCourses[0];
        ...
        if (compatibility.status === "blocked") {
          ...
          setLearningView(requestedRoute.view);
        }
      }
    }
    void hydrate().catch(() => {
  ...
  useEffect(() => {
    if (!hydrated || space !== "learn") return;
    const syncFromUrl = () => {
      const requestedRoute = readLearnRoute(window.location.search);
      const requestedCourse = installedCourses.find((item) => item.manifest.id === requestedRoute.courseId);
      if (requestedCourse && requestedCourse.manifest.id !== course.manifest.id) {
        setCourse(requestedCourse);
        setSource(JSON.stringify(requestedCourse, null, 2));
        setLanguage(requestedCourse.manifest.languageId);
      }
      setLearningView(requestedRoute.view);
    };
    window.addEventListener("popstate", syncFromUrl);
    return () => window.removeEventListener("popstate", syncFromUrl);
  }, [course.manifest.id, hydrated, installedCourses, space]);

  useEffect(() => {
    if (!hydrated || space !== "learn") return;
    const view = learningView === "library" || learningView === "settings" || learningView === "plan" || learningView === "dashboard" ? learningView : "dashboard";
    const url = writeLearnRoute(window.location.href, {
      view,
      courseId: installedCourses.some((item) => item.manifest.id === course.manifest.id) ? course.manifest.id : undefined,
    });
    window.history.replaceState(null, "", url);
  }, [course.manifest.id, hydrated, installedCourses, learningView, space]);
  ...
  const renderLibraryScreen = (settingsMode = false) => <CourseLibrary
    ...
    onCreateCourse={() => router.push("/studio")}
  ...
  return (
    <main className={`studio-shell ${!studioStarted ? "studio-start-mode" : ""}`}>
        ...
        <div className="work-grid">
          <section className="editor-panel panel">
            <div className="panel-heading editor-heading">
              ...
              <div className="mode-switch" role="group" aria-label={t("编辑方式", "Editing mode")}>
                <button aria-pressed={editorMode === "visual"} className={editorMode === "visual" ? "active" : ""} onClick={() => setEditorMode("visual")}><BookOpen size={14} />{t("可视化", "Visual")}</button>
                <button aria-pressed={editorMode === "json"} className={editorMode === "json" ? "active" : ""} onClick={() => setEditorMode("json")}><Braces size={14} />JSON</button>
                {editorMode === "visual" && <button aria-pressed={showAdvanced} className={showAdvanced ? "active" : ""} onClick={() => setShowAdvanced((value) => !value)}><SlidersHorizontal size={14} />{t("进阶字段", "Advanced")}</button>}
              </div>
            </div>
            ...
            {editorMode === "visual" ? (
              <>
                <div className="section-tabs" role="group" aria-label={t("课程内容分区", "Course content sections")}>
                  {sectionLabels.map(([id, chinese, english]) => <button key={id} aria-pressed={editorSection === id} className={editorSection === id ? "active" : ""} onClick={() => setEditorSection(id)}>{t(chinese, english)}</button>)}
                </div>
                <div className="visual-editor" id="studio-visual-editor" tabIndex={-1}>
                  {editorSection === "overview" && (
                    <div className="form-section">
                      ...
                      <div className="form-grid two-column">
                        ...
                        <label className="wide"><span>{t("课程名称", "Course title")}（{teachingLocale === "en" ? "English" : "中文"}）</span><input readOnly={course.manifest.status === "published"} id="studio-course-title" value={course.manifest.title[teachingLocale] ?? ""} onChange={(event) => editCourse((next) => { next.manifest.title[teachingLocale] = event.target.value; })} /></label>
                        <label className="wide"><span>{t("课程简介", "Course description")}（{teachingLocale === "en" ? "English" : "中文"}）</span><textarea readOnly={course.manifest.status === "published"} id="studio-course-description" value={course.manifest.description[teachingLocale] ?? ""} onChange={(event) => editCourse((next) => { next.manifest.description[teachingLocale] = event.target.value; })} /></label>
                  ...
                  {(editorSection === "knowledge" || editorSection === "utterances" || editorSection === "exercises") && (
                    <CourseContentEditor
                      course={course}
                      editorSection={editorSection}
                      locale={appLocale}
                      direction={currentLanguage?.scripts[0]?.direction ?? "ltr"}
                  ...
                  {editorSection === "flow" && (
                    <CourseFlowEditor course={course} locale={appLocale} selectedUnitId={selectedUnitId} selectedStudioLessonId={selectedStudioLessonId} setSelectedUnitId={setSelectedUnitId} setSelectedStudioLessonId={setSelectedStudioLessonId} editCourse={editCourse} setNotice={setNotice} />
                  )}
                </div>
              </>
            ) : (
      ...
      {articleOpen && <MaterialImportDialog locale={appLocale} form={articleForm} materials={articleMaterials} busy={articleBusy} error={articleError} aiConfigured={aiConfigured} languagePacks={languagePacks} setForm={setArticleForm} setError={setArticleError} onClose={() => setArticleOpen(false)} onAddPastedMaterial={addPastedMaterial} onImportFiles={(files) => void importMaterialFiles(files)} onImportUrl={() => void importMaterialUrl()} onRemoveMaterial={(id) => setArticleMaterials((current) => current.filter((item) => item.id !== id))} onCreate={() => void createArticleCourse()} />}
