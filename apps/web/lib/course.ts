...
import { canonicalizeCourse, validateCourse } from "@learn-language/content/course-validation";
...
export type { ImportIssue } from "@learn-language/protocol";
...
export async function calculateCourseHash(course: CoursePack): Promise<string> {
  const bytes = new TextEncoder().encode(canonicalizeCourse(course));
