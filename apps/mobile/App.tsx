  ...
  return (
    <ScrollView contentContainerStyle={styles.scrollContent}>
      ...
      <View style={styles.sectionHeading}>
        ...
        <Text style={styles.sectionNote}>{copy(locale, "内置课程可离线学习", "Built-in courses work offline")}</Text>
      </View>
      {courses.map((course) => {
      ...
      <View style={styles.privacyCard}>
        ...
        <Text style={styles.privacyText}>{copy(locale, "首版移动端不依赖云端。没有网络时，已内置课程与本地进度仍可使用。", "The first mobile release does not depend on cloud services. Built-in courses and local progress remain available offline.")}</Text>
      </View>
    </ScrollView>
  );
}
...
function Settings({
  ...
  const reset = () => Alert.alert(
    ...
    [
      ...
      { text: copy(locale, "删除", "Delete"), style: "destructive", onPress: () => void run(async () => { await repository.clear(); await onReload(); setNotice(copy(locale, "本机学习数据已删除", "Local learning data deleted")); }) },
    ],
  );
  return (
    <ScrollView contentContainerStyle={styles.scrollContent}>
      ...
      <View style={styles.settingsCard}>
        ...
        <View style={styles.stackActions}>
          ...
          <Button label={copy(locale, "选择备份恢复", "Choose backup to restore")} onPress={() => void run(async () => { const result = await importMobileBackup(repository); if (result) { await onReload(); setNotice(copy(locale, `恢复完成：新增 ${result.added}，更新 ${result.replaced}，跳过 ${result.skipped}`, `Restore complete: ${result.added} added, ${result.replaced} updated, ${result.skipped} skipped`)); } })} tone="secondary" />
        </View>
      </View>
      <View style={styles.settingsCard}>
        <Text style={styles.settingsTitle}>{copy(locale, "本地存储", "Local storage")}</Text>
        <Text style={styles.settingsText}>{copy(locale, "SQLite 保存课程进度。AI 密钥、登录凭据和临时请求不会进入备份。", "SQLite stores progress. AI keys, sign-in credentials, and temporary requests are excluded from backup.")}</Text>
...
function MobileApp() {
  ...
  const repository = useMemo(() => new SQLiteLearningProfileRepository(db), [db]);
  ...
  const reload = useCallback(async () => {
    ...
    setRecords(Object.fromEntries(loaded.map((record) => [record.courseId, record])));
  }, [repository]);

  useEffect(() => {
    void Promise.all([repository.list(), preferences.locale()]).then(([loaded, storedLocale]) => {
      setRecords(Object.fromEntries(loaded.map((record) => [record.courseId, record])));
  ...
  const saveRecord = async (record: CourseLearningRecord) => {
    await repository.putMany([record]);
  ...
  if (screen.kind === "lesson" && activeCourse && progress) {
    ...
    content = <Reviews locale={locale} records={records} onUpdate={saveRecord} />;
  } else if (screen.kind === "settings") {
    content = <Settings locale={locale} records={records} repository={repository} onReload={reload} onLocale={async (nextLocale) => { await preferences.setLocale(nextLocale); setLocale(nextLocale); }} />;
  } else {
    content = <Home locale={locale} records={records} onCourse={(courseId) => setScreen({ kind: "course", courseId })} />;
  }
